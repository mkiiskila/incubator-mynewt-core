...
struct hal_uart {
    ...
    void *u_func_arg;
    const struct stm32l0_uart_cfg *u_cfg;
};
...
{
    struct hal_uart *u;
    const struct stm32l0_uart_cfg *cfg;
    ...
    *cfg->suc_rcc_reg |= cfg->suc_rcc_dev;
    ...
    if (cfg->suc_uart == USART1) {
        u->u_regs->BRR = UART_DIV_SAMPLING16(HAL_RCC_GetPCLK2Freq(), baudrate);
    } else {
        u->u_regs->BRR = UART_DIV_SAMPLING16(HAL_RCC_GetPCLK1Freq(), baudrate);
    }
...
hal_uart_init(int port, void *arg)
{
    ...
    u = &uarts[port];
    u->u_cfg = (const struct stm32l0_uart_cfg *)arg;
