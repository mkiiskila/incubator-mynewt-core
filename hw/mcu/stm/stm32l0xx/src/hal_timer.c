...
#define STM32L0_HAL_TIMER_MAX     (2)

struct stm32l0_hal_tmr {
...
struct stm32l0_hal_tmr stm32l0_tmr0;
#endif
#if MYNEWT_VAL(TIMER_1)
struct stm32l0_hal_tmr stm32l0_tmr1;
#endif

static struct stm32l0_hal_tmr *stm32l0_tmr_devs[STM32L0_HAL_TIMER_MAX] = {
#if MYNEWT_VAL(TIMER_0)
    &stm32l0_tmr0,
#else
    NULL,
...
static uint32_t hal_timer_cnt(struct stm32l0_hal_tmr *tmr);
 ...
 */
static void
stm32l0_tmr_cbs(struct stm32l0_hal_tmr *tmr)
{
    uint32_t cnt;
    struct hal_timer *ht;
 ...
 */
static void
stm32l0_tmr_irq(struct stm32l0_hal_tmr *tmr)
{
    uint32_t sr;
    uint32_t clr = 0;
...
#if MYNEWT_VAL(TIMER_0)
static void
stm32l0_tmr0_irq(void)
{
    stm32l0_tmr_irq(&stm32l0_tmr0);
}
#endif

#if MYNEWT_VAL(TIMER_1)
static void
stm32l0_tmr1_irq(void)
{
    stm32l0_tmr_irq(&stm32l0_tmr1);
}
#endif

static void
stm32l0_tmr_reg_irq(IRQn_Type irqn, uint32_t func)
{
...
static uint32_t
stm32l0_base_freq(TIM_TypeDef *regs)
{
...
static void
stm32l0_hw_setup(int num, TIM_TypeDef *regs)
{
    uint32_t func;
...
#if MYNEWT_VAL(TIMER_0)
    case 0:
        func = (uint32_t)stm32l0_tmr0_irq;
        break;
#endif
#if MYNEWT_VAL(TIMER_1)
    case 1:
        func = (uint32_t)stm32l0_tmr1_irq;
        break;
#endif
    default:
        assert(0);
...
#ifdef TIM2
    if (regs == TIM2) {
        stm32l0_tmr_reg_irq(TIM2_IRQn, func);
        __HAL_RCC_TIM2_CLK_ENABLE();
    }
#endif
#ifdef TIM3
    if (regs == TIM3) {
        stm32l0_tmr_reg_irq(TIM3_IRQn, func);
        __HAL_RCC_TIM3_CLK_ENABLE();
    }
#endif
#ifdef TIM6
    if (regs == TIM6) {
        stm32l0_tmr_reg_irq(TIM6_DAC_IRQn, func);
        __HAL_RCC_TIM6_CLK_ENABLE();
    }
#endif
#ifdef TIM7
    if (regs == TIM7) {
        stm32l0_tmr_reg_irq(TIM7_IRQn, func);
...
static void
stm32l0_hw_setdown(TIM_TypeDef *regs)
{
#ifdef TIM2
    if (regs == TIM2) {
...
{
    struct stm32l0_hal_tmr *tmr;

    if (num >= STM32L0_HAL_TIMER_MAX || !(tmr = stm32l0_tmr_devs[num]) ||
        (cfg == NULL)) {
        return -1;
    }
    ...
    stm32l0_hw_setup(num, tmr->sht_regs);

    /*
     * Stop the timers at debugger. XXX Which TIM?
     */
    DBGMCU->APB1FZ |= 0x33; /* TIM2 - TIM3, TIM6-TIM7 */
...
hal_timer_config(int num, uint32_t freq_hz)
{
    ...
    prescaler = stm32l0_base_freq(tmr->sht_regs) / freq_hz;
    if (prescaler > 0xffff) {
        return -1;
    }
...
hal_timer_deinit(int num)
{
    ...
    stm32l0_hw_setdown(tmr->sht_regs);
...
static uint32_t
hal_timer_cnt(struct stm32l0_hal_tmr *tmr)
{
    uint32_t cnt;
    int sr;
...
hal_timer_start(struct hal_timer *timer, uint32_t ticks)
{
    ...
    tmr = (struct stm32l0_hal_tmr *)timer->bsp_timer;
