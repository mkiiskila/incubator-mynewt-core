...
#define STM32L0_HAL_SPI_TIMEOUT (1000)

#define STM32L0_HAL_SPI_MAX (3)

struct stm32l0_hal_spi {
    ...
    struct stm32l0_hal_spi_cfg *cfg;
    /* Callback and arguments */
    hal_spi_txrx_cb txrx_cb_func;
    void            *txrx_cb_arg;
};

static struct stm32l0_spi_stat {
    uint32_t irq;
    uint32_t ss_irq;
    uint32_t tx;
    uint32_t eovf;
...
static void spi_irq_handler(struct stm32l0_hal_spi *spi);

#if MYNEWT_VAL(SPI_0)
struct stm32l0_hal_spi stm32l0_hal_spi0;
#endif
#if MYNEWT_VAL(SPI_1)
struct stm32l0_hal_spi stm32l0_hal_spi1;
#endif
#if MYNEWT_VAL(SPI_2)
struct stm32l0_hal_spi stm32l0_hal_spi2;
#endif

static struct stm32l0_hal_spi *stm32l0_hal_spis[STM32L0_HAL_SPI_MAX] = {
#if MYNEWT_VAL(SPI_0)
    &stm32l0_hal_spi0,
#else
    NULL,
...
#define STM32L0_HAL_SPI_RESOLVE(__n, __v)                        \
    if ((__n) >= STM32L0_HAL_SPI_MAX) {                          \
        rc = -1;                                                 \
        goto err;                                                \
    }                                                            \
    (__v) = (struct stm32l0_hal_spi *) stm32l0_hal_spis[(__n)];  \
    if ((__v) == NULL) {                                         \
        rc = -1;                                                 \
        goto err;                                                \
    }
...
static IRQn_Type
stm32l0_resolve_spi_irq(SPI_HandleTypeDef *hspi)
{
 ...
 */
static void
spim_irq_handler(struct stm32l0_hal_spi *spi)
{
 ...
 */
static void
spis_irq_handler(struct stm32l0_hal_spi *spi)
{
    if (spi->tx_in_prog) {
        if (spi->handle.TxXferCount == 0 && spi->handle.RxXferCount == 0) {
            /*
 ...
 */
static void
spi_irq_handler(struct stm32l0_hal_spi *spi)
{
    uint32_t err;
...
spi_ss_isr(void *arg)
{
    struct stm32l0_hal_spi *spi = (struct stm32l0_hal_spi *)arg;
    int ss;
    int len;
    uint16_t reg;
...
{
    spi_irq_handler(stm32l0_hal_spis[0]);
}

static void
spi2_irq_handler(void)
{
    spi_irq_handler(stm32l0_hal_spis[1]);
}


uint32_t
stm32l0_resolve_spi_irq_handler(SPI_HandleTypeDef *hspi)
{
...
hal_spi_init(int spi_num, void *usercfg, uint8_t spi_type)
{
     ...
     */
    STM32L0_HAL_SPI_RESOLVE(spi_num, spi);
...
static int
stm32l0_spi_resolve_prescaler(uint8_t spi_num, uint32_t baudrate, uint32_t *prescaler)
{
    uint32_t candidate_br;
    uint32_t apbfreq;
    int i;

    /* SPIx {1,4,5,6} use PCLK2 on the STM32L0, otherwise use PCKL1.
...
hal_spi_config(int spi_num, struct hal_spi_settings *settings)
{
    ...
    rc = stm32l0_spi_resolve_prescaler(spi_num, settings->baudrate * 1000,
      &prescaler);
    if (rc != 0) {
        goto err;
    }
    ...
    irq = stm32l0_resolve_spi_irq(&spi->handle);
    NVIC_SetPriority(irq, cfg->irq_prio);
    NVIC_SetVector(irq, stm32l0_resolve_spi_irq_handler(&spi->handle));
    NVIC_EnableIRQ(irq);
