...
#define FLASH_FLAG_MASK                                                 \
    (FLASH_FLAG_EOP | FLASH_FLAG_ENDHV | FLASH_FLAG_WRPERR |            \
      ...
      FLASH_SR_RDERR | FLASH_FLAG_FWWERR | FLASH_FLAG_NOTZEROERR)
...
/*
 * Flash size depends on MCU type. XXXX read from 0x1FF8007C.
 */
...
stm32l0_flash_erase_sector(const struct hal_flash *dev, uint32_t sector_address)
{
    ...
    if (!(sector_address & (_FLASH_SECTOR_SIZE - 1))) {
        ...
        eraseinit.PageAddress = sector_address - dev->hf_base_addr;
